...
def putdata(session, stock_entry):
    try:
        ...
        price = float(entry.get('predication'))
        average = float(entry.get('average'))
        datetime = float(entry.get('timestamp'))
        statement = "insert into %s (stock_symbol, last_trade_date_time, prediction_price, average_price) values ('%s', %f, %f, %f)" % (table, symbol, datetime, price, average)
        session.execute(statement)
        logger.info('put data successfully to cassandra for stock_symbol: %s, last_trade_data_time: %f, price: %f, average: %f' % (symbol, datetime, price, average))
    except Exception,e:
        logger.error('failed to put data to cassandra %s', e.message)
...
if __name__ == '__main__':
    ...
    session.execute("create table if not exists %s (stock_symbol text, last_trade_date_time float, prediction_price float, average_price float, PRIMARY KEY (stock_symbol,last_trade_date_time))" % table)
